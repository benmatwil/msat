...
test3=0
do i=1,3
  if (test2(2*i-1) .ge. 0) then !if there is a crossing
    if (abs(test2(2*i)) .lt. test2(2*i-1)) test3(i) =1
  endif
enddo


if (sum(test3) .eq. 3) inull=1 !we have a null (in the cell)

! check for null on surface of cell
if ((sum(test(:,2)) .gt. 50) .or. (sum(test(:,4)) .gt. 50) .or.(sum(test(:,6)) .gt. 50)) then
...
do j=1,2
  do i=1,2
    if (a(i,j) .gt. zero) then
      totala=totala+1
    else if (a(i,j) .lt. -zero) then
      totala=totala-1
    else
      totala=totala+100
    endif

    if (b(i,j) .gt. zero) then
      totalb=totalb+1
    else if (b(i,j) .lt. -zero) then
      totalb=totalb-1
    else
      totalb=totalb+100
    endif
...
if ( (abs(totala) .eq. 4) .and. (abs(totalb) .eq. 4) ) then
  zeropoint = .false.
else
  zeropoint = .true.
endif
...
logical :: blankface
 ...
 if (atot + btot + ctot .le. zero*12.d0) then
   !print*, 'BLANK FACE'
   blankface =.false.
 endif
...
logical function check(x,y)
  ...
  check= (x .ge. 0.) .and. (x .le. 1.) .and. (y .ge. 0.) .and. (y .le. 1.)
 end
...
!determinant of quadratic
  ...
  if (det .ge. 0.) then !there is a solution

    if (abs(a) .lt. zero) then !have to solve linear


      if (b .eq. 0.) then !no solution exists
         x1=-1.
         y1=-1.
    ...
    if (check(x1,y1)) then !if x and y are on face (between 0 and 1)
      ...
      if (zcomp .gt. zero) then
        sign=sign+1
      else if (zcomp .lt. -zero) then
        sign=sign-1
      else
        sign=sign+100
      endif
...
!check for null along edges of a cell face
   ...
   if (edge_check .gt. 0) edge_check=1 !if nulls are found, set the counter to 1
...
!checks if null lies on an edge
 ...
 ! double precision :: x, y, z
  ...
  if ((x1 .eq. 0. .and. x2 .eq. 0.) .and. (y1 .eq. 0. .and. y2 .eq. 0.)) then
     if (z1*z2 .le. 0.) line_check=1
  else if ((x1 .eq. 0. .and. x2 .eq. 0.) .and. (z1 .eq. 0. .and. z2 .eq. 0.)) then
     if (y1*y2 .le. 0.) line_check=1
  else if ((z1 .eq. 0. .and. z2 .eq. 0.) .and. (y1 .eq. 0. .and. y2 .eq. 0.)) then
     if (x1*x2 .le. 0.) line_check=1
  else
     line_check=0
  endif
...
do while (b .gt. 1.e-4 .or. diff .lt. 1.e-4)
icounter=icounter+1
xold=x
yold=y
zold=z
...
integer function switch(cube)
  !determines whether all the vertices of the cube have the same sign or not
    ...
    do k=1,2
      do j=1,2
        do i=1,2
          if(cube(i,j,k) .gt. zero) then
             icount=icount+1
          else if (cube(i,j,k) .lt. -zero) then
             icount=icount-1
          else
             icount=icount+0
             !print *, 'ZERO'
    ...
    if (abs(icount) .lt. 8) then !don't all have same sign
       itest=1
...
do k=1,11
  ...
  do j=1,11
    ...
    do i=1,11
       ...
       !if (k .eq. 1 .and. j .eq. 1) print*,x,y,z
    enddo
...
outer: do k=1,10
  do j=1,10
   do i=1,10
    ...
    ! print*, xs+dx*(i-1), ys+dx*(j-1), zs+dx*(k-1)
   ...
   if ((k .eq. 10) .and. (j .eq. 11) .and. (i .eq. 11)) then
...
do while (range .gt. 10.**(-sig_figs)) !num=1,sig_figs
        range=range/5.
        dx=range/10
      !  print*,range,dx,xs,xs+10*dx
...
outer: do while (exitcondition .eq. 0)
