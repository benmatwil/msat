...
def add_structures(*args, **kwargs):
    ...
    if 'sepsurf_flines' in args: add_sepsurf(draw='fieldlines', nlines=kwargs['nfanlines'], nring=kwargs['nring'])
    if 'spines' in args: add_spines()
    if 'sepsurf_rings' in args: add_sepsurf()
    if 'separators' in args: add_separators()
    if 'hcs_rings' in args: add_hcs()
    if 'hcs_flines' in args: add_hcs(draw='fieldlines')
...
def add_base(coord, pos, vmin=-10, vmax=10):
    ...
    if coord == "x":
        ...
        ml.mesh(x, y, z, scalars=-bgrid[ipos, :, :, 0], colormap='Greys', vmin=vmin, vmax=vmax)
    elif coord == "y":
        x, z = np.meshgrid(xx, zz, indexing='ij')
        y = np.ones_like(x)*yy[ipos]
        ml.mesh(x, y, z, scalars=-bgrid[:, ipos, :, 1], colormap='Greys', vmin=vmin, vmax=vmax)
    elif coord == "z":
