...
\begin{document}
  ...
  \section{Preparing the data}
    ...
    Since Python uses C memory order, arrays must be transposed in NumPy to switch them to Fortran order for writing. There is also the option to create arrays in NumPy in Fortran order using the \texttt{order=`F'} keyword argument in the following NumPy command \texttt{numpy.array((nx, ny, nz), dtype=numpy.float64, order=`F')}. However I do not know whether this has the required effect when writing to file.

    NumPy's meshgrid function makes it easy to create grids from analytic functions. For 3D grids, you need add the keyword argument \texttt{indexing=`ij'} to keep the array sizing correct.
    ...
    \subsection{3D Visualisation}
      ...
      \begin{center}
        \begin{tikzpicture}[remember picture,overlay]
          \node at (0,0) {\texttt{seps\tikzmark{inull}[inull][isep]}};
          \filldraw[fill=black] (pic cs:inull.south west) circle[radius=0.08cm] node[above] {a};
          \filldraw[fill=black] (pic cs:inull.south east) circle[radius=0.08cm] node[above] {b};
          \filldraw[fill=black] (pic cs:inull.north west) circle[radius=0.08cm] node[above] {c};
          \filldraw[fill=black] (pic cs:inull.north east) circle[radius=0.08cm] node[above] {d};
          \filldraw[fill=black] (pic cs:inull) circle[radius=0.08cm] node[above] {e};
        \end{tikzpicture}
      \end{center}
